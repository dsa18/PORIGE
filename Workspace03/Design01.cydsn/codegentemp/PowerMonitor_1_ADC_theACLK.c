/*******************************************************************************
* File Name: PowerMonitor_1_ADC_theACLK.c
* Version 1.70
*
*  Description:
...
/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_Start
********************************************************************************
* Summary:
...
void PowerMonitor_1_ADC_theACLK_Start(void) 
{
    ...
    PowerMonitor_1_ADC_theACLK_CLKEN |= PowerMonitor_1_ADC_theACLK_CLKEN_MASK;
}


/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_Stop
********************************************************************************
* Summary:
...
/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_StopBlock
********************************************************************************
* Summary:
...
{
    if (PowerMonitor_1_ADC_theACLK_CLKEN & PowerMonitor_1_ADC_theACLK_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = PowerMonitor_1_ADC_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PowerMonitor_1_ADC_theACLK_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(PowerMonitor_1_ADC_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PowerMonitor_1_ADC_theACLK_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_StandbyPower
********************************************************************************
* Summary:
...
void PowerMonitor_1_ADC_theACLK_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        PowerMonitor_1_ADC_theACLK_CLKSTBY &= ~PowerMonitor_1_ADC_theACLK_CLKSTBY_MASK;
    }
    else
    {
        PowerMonitor_1_ADC_theACLK_CLKSTBY |= PowerMonitor_1_ADC_theACLK_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_SetDividerRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void PowerMonitor_1_ADC_theACLK_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    uint8 enabled;

    uint8 currSrc = PowerMonitor_1_ADC_theACLK_GetSourceRegister();
    uint16 oldDivider = PowerMonitor_1_ADC_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PowerMonitor_1_ADC_theACLK_CLKEN & PowerMonitor_1_ADC_theACLK_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(PowerMonitor_1_ADC_theACLK_DIV_PTR, clkDivider);
                PowerMonitor_1_ADC_theACLK_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
        ...
        else
        {
            ...
            PowerMonitor_1_ADC_theACLK_CLKEN |= enabled;
        }
    }
}
...
/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_GetDividerRegister
********************************************************************************
* Summary:
...
{
    return CY_GET_REG16(PowerMonitor_1_ADC_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void PowerMonitor_1_ADC_theACLK_SetModeRegister(uint8 clkMode) 
{
    PowerMonitor_1_ADC_theACLK_MOD_SRC |= clkMode & PowerMonitor_1_ADC_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void PowerMonitor_1_ADC_theACLK_ClearModeRegister(uint8 clkMode) 
{
    PowerMonitor_1_ADC_theACLK_MOD_SRC &= ~clkMode | ~PowerMonitor_1_ADC_theACLK_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_GetModeRegister
********************************************************************************
* Summary:
...
{
    return PowerMonitor_1_ADC_theACLK_MOD_SRC & PowerMonitor_1_ADC_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = PowerMonitor_1_ADC_theACLK_GetDividerRegister();
    uint8 oldSrc = PowerMonitor_1_ADC_theACLK_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        PowerMonitor_1_ADC_theACLK_MOD_SRC |= CYCLK_SSS;
        PowerMonitor_1_ADC_theACLK_MOD_SRC =
            (PowerMonitor_1_ADC_theACLK_MOD_SRC & ~PowerMonitor_1_ADC_theACLK_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return PowerMonitor_1_ADC_theACLK_MOD_SRC & PowerMonitor_1_ADC_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    PowerMonitor_1_ADC_theACLK_PHASE = clkPhase & PowerMonitor_1_ADC_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PowerMonitor_1_ADC_theACLK_GetPhase
********************************************************************************
* Summary:
...
{
    return PowerMonitor_1_ADC_theACLK_PHASE & PowerMonitor_1_ADC_theACLK_PHASE_MASK;
}
