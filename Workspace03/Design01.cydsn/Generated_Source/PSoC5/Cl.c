...
void Cl_Start(void) 
{
    ...
    Cl_CLKEN |= Cl_CLKEN_MASK;
	Cl_CLKSTBY |= Cl_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Cl_Stop
********************************************************************************
*
* Summary:
...
void Cl_Stop(void) 
{
    ...
    Cl_CLKEN &= (uint8)(~Cl_CLKEN_MASK);
	Cl_CLKSTBY &= (uint8)(~Cl_CLKSTBY_MASK);
}
...
void Cl_StopBlock(void) 
{
    if ((Cl_CLKEN & Cl_CLKEN_MASK) != 0u)
    {
...
#if defined(Cl__CFG3)
        CLK_DIST_AMASK = Cl_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Cl_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Cl_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Cl_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Cl_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Cl_GetSourceRegister();
    uint16 oldDivider = Cl_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Cl_CLKEN & Cl_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Cl_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Cl_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Cl_DIV_PTR);
}


/*******************************************************************************
* Function Name: Cl_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Cl_MOD_SRC |= modeBitMask & (uint8)Cl_MODE_MASK;
}


/*******************************************************************************
* Function Name: Cl_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Cl_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Cl_MODE_MASK));
}


/*******************************************************************************
* Function Name: Cl_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Cl_MOD_SRC & (uint8)(Cl_MODE_MASK);
}


/*******************************************************************************
* Function Name: Cl_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Cl_GetDividerRegister();
    uint8 oldSrc = Cl_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Cl_MOD_SRC |= CYCLK_SSS;
        Cl_MOD_SRC =
            (Cl_MOD_SRC & (uint8)(~Cl_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Cl_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Cl_MOD_SRC & Cl_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Cl_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Cl_PHASE = clkPhase & Cl_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Cl_GetPhase
********************************************************************************
*
* Summary:
